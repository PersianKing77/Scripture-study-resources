...
// ---------- index ----------
const index = el('idx-places');
function collapseToggleBtn(getGroups) {
  const btn = document.createElement('button');
  btn.className = 'idx-allbtn';
  const sync = () => {
    const gs = getGroups();
    btn.textContent = gs.length && gs.every(g => g.classList.contains('collapsed')) ? 'Expand all' : 'Collapse all';
  };
  btn.addEventListener('click', () => {
    const gs = getGroups();
    const collapseNow = !gs.every(g => g.classList.contains('collapsed'));
    gs.forEach(g => g.classList.toggle('collapsed', collapseNow));
    sync();
  });
  btn.sync = sync;
  return btn;
}
const placesToggle = collapseToggleBtn(() => Array.from(index.querySelectorAll('.idx-group')));
index.appendChild(placesToggle);
GROUPS.forEach(g => {
  const wrap = document.createElement('div'); wrap.className = 'idx-group collapsed';
  const h = document.createElement('h4'); h.textContent = g.name;
  h.addEventListener('click', () => { wrap.classList.toggle('collapsed'); placesToggle.sync(); });
  wrap.appendChild(h);
  const body = document.createElement('div'); body.className = 'idx-group-body';
  g.ids.forEach(id => {
...
// Dates are proleptic Gregorian equivalents of the festival days in c. AD 30, and the
...
const SOLAR = [
  { name: 'Passover dawn', date: [30, 3, 5], h: 6.3, sub: '15 Nisan, c. AD 30 — the hour of the morning sacrifice' },
...
addEventListener('keydown', e => {
  ...
  if (e.code === 'Escape') {
    if (el('handout-overlay').style.display === 'flex') closeHandout();
    else closeLabel();
  }
...
function tryMove(dx, dz) {
  ...
  for (const [ax, az] of attempts) {
    ...
    // a single frame can cover several stair treads at once (bigger strides on slow
    // devices, running, etc); STEP_UP alone would then read a staircase as a wall, so
    // the allowance grows with how far this stride actually travelled.
    if (fy - walk.pos.y > STEP_UP + len * 0.6) continue;
...
el('m-places2').addEventListener('click', () => { setSidebarMode('places'); el('tools').classList.remove('open'); sheet('index'); });
el('index').addEventListener('click', ev => {
  if (isNarrow() && ev.target.closest('button') && !ev.target.closest('.idx-allbtn, #cfm-print')) {
...
Object.entries(CERT_LEVELS).forEach(([k, v]) => {
  certMats[k] = new THREE.MeshStandardMaterial({
    color: 0xffffff, roughness: 0.85, metalness: 0, side: THREE.DoubleSide, name: 'certainty_' + k,
    map: certPatternTexture(CERT_PATTERNS[k], v.tint)   // tint is baked into the map; don't multiply it again via material.color
  });
});
let certOn = false;
...
function addMeasurePoint(ndc) {
  ...
  if (measure.pts.length === 2) {
    ...
    el('measure-out').innerHTML =
      `<b>${d.toFixed(1)} m &middot; ${(d * 3.2808).toFixed(1)} ft &middot; ${(d / 0.5).toFixed(0)} cubits</b>` +
      `<span>horizontal ${dh.toFixed(1)} m (${(dh * 3.2808).toFixed(1)} ft) &middot; vertical ${dv.toFixed(1)} m (${(dv * 3.2808).toFixed(1)} ft)</span>` +
...
function visibleModal() {
  const candidates = [el('panel'), el('about-overlay'), el('handout-overlay'), el('welcome')];
...
el('cfm-body').innerHTML = CURRICULUM.map(vol =>
  `<div class="idx-group collapsed"><h3>${vol.vol}</h3><div class="idx-group-body">` + vol.blocks.map((b, i) =>
...
const studyToggle = collapseToggleBtn(() => Array.from(el('cfm-body').querySelectorAll('.idx-group')));
el('cfm-body').before(studyToggle);
studyToggle.sync();
el('cfm-body').addEventListener('click', ev => {
  const h3 = ev.target.closest('h3');
  if (h3) { h3.closest('.idx-group').classList.toggle('collapsed'); studyToggle.sync(); return; }
...
let sidebarMode = 'places';
// Places and Study share one sidebar slot; this keeps the tab label, the two toolbar
// buttons' "on" state, and the visible content (#idx-places / #idx-study) all in sync.
function setSidebarMode(m) {
  sidebarMode = m;
  document.body.classList.toggle('study-mode', m === 'study');
  const off = document.body.classList.contains('index-off');
  const label = m === 'study' ? 'Study' : 'Places';
  idxHandle.innerHTML = off ? `<span>${label}</span>\u203a` : `<span>${label}</span>\u2039`;
  const verb = off ? 'Show ' : 'Hide ';
  const what = m === 'study' ? 'the weekly study index' : 'the list of places';
  idxHandle.title = verb + what;
  idxHandle.setAttribute('aria-label', verb + what);
  el('m-places').classList.toggle('on', !off && m === 'places');
  el('cfm-btn').classList.toggle('on', !off && m === 'study');
}
...
function openSidebar(m) {
  const off = document.body.classList.contains('index-off');
  if (!off && sidebarMode === m) { setIndexOff(true); return; }
  setSidebarMode(m);
  if (off) setIndexOff(false);
}
idxHandle.addEventListener('click', () => setIndexOff(!document.body.classList.contains('index-off')));
el('m-places').addEventListener('click', () => openSidebar('places'));
el('cfm-btn').addEventListener('click', () => {
  if (isNarrow()) { setSidebarMode('study'); el('tools').classList.remove('open'); sheet('index'); }
  else openSidebar('study');
});
