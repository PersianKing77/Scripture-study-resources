...
(function () {
  /* Crest profile in metres above sea level, station 0 = south wall of the Ophel / Temple Mount
     ...
     Ophel end of the ridge c. 725 m, Area G / Stepped Stone Structure c. 702–706 m, the crest opposite
     the Gihon c. 685 m (the spring itself is at 636 m, far down the eastern flank). At the southern
     tip the tunnel outlet and pool water level are c. 636 m; the excavated pool floors sit a few
     metres lower, at c. 632–633 m. */
  ...
  var B = [
    ...
    { per:'herod', n:'Monumental stepped street — commonly interpreted as a pilgrimage route, c. 8 m wide', cat:'street', conf:'secure', line:[[652,630],[540,644],[420,661],[300,679],[180,693],[70,712]], th:1.2, z3:-34, w3:8, lbl:1 },
    ...
    { per:'islam', n:'Earthquake-collapse horizon (commonly dated AD 749)', cat:'destruction', conf:'probable', x:96, w:30, h:2.2, dz:9, lbl:1 },
    ...
    { per:'modern', n:'Monumental stepped street tunnel excavation', cat:'excav', conf:'secure', line:[[652,630],[540,644],[420,661],[300,679]], th:3.4, z3:-34, lbl:1 },
    { per:'modern', n:'Visitor entrance', cat:'modern', conf:'secure', x:126, w:9, h:6, dz:0 }
  ];

  var FIGURES = [250, 470, 640];

  /* The pool's water level is not an independent figure: it is the far end of Hezekiah's
     Tunnel, and the difference from the spring is the tunnel's whole fall. Derived here so
     the number cannot be retyped inconsistently across the views. */
  var HEZ = B.filter(function (b) { return b.line && /hezekiah/i.test(b.n || ''); })[0];
  var POOL = HEZ ? HEZ.line[HEZ.line.length - 1] : [664, 635.7];
  var SPRING_E = 636;
  ...
  window.COD_ELEV = {
    ...
    spring: { x: 298, elev: SPRING_E, z: 79 },
    pool: { x: POOL[0], elev: POOL[1] },
    tunnelFallCm: Math.round((SPRING_E - POOL[1]) * 100),
    landmarks: [
      { x: 20, elev: 730, t: 'NORTH — Ophel, Temple Mount 740 m', a: 'start' },
      { x: 298, elev: 641, t: 'Gihon Spring · ' + SPRING_E + ' m', a: 'middle' },
      { x: 660, elev: 646, t: 'Pool of Siloam · ' + POOL[1] + ' m', a: 'middle' },
