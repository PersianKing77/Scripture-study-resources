...
(function () {
  ...
  // Reconstruction from published excavation plans (Mauss's clearance from 1863, published
  // 1888; École Biblique campaigns of 1957-62 under de Vaux, Rousée and Duprez; later
  // re-examinations). Heights are informed estimates.

  const CENTER = { x: 84, y: 74 };
  ...
  // Seven-tier evidence-status grammar (SCH-05/10/16). Every modeled feature
  // carries a tier, either the category default below or an explicit override
  // on the feature itself, so the grammar covers every phase automatically.
  // Patterns (not just color) carry the distinction — see the matching SVG
  // <pattern> defs in Pool of Bethesda.dc.html and the CSS swatches in its
  // legend, both keyed by these same seven ids.
  const EVIDENCE_TIERS = [
    { id: "excavated",    name: "Excavated in situ",           desc: "Directly observed in the excavation record." },
    { id: "secured",      name: "Structurally secured",        desc: "Position fixed by an observed, attached structure." },
    { id: "textual",      name: "Textually attested",          desc: "Named or counted in an ancient source; plan reconstructed." },
    { id: "inferred",     name: "Inferred from parallels",     desc: "Typical of the period and region; no direct local evidence." },
    { id: "conventional", name: "Conventional elevation",       desc: "A drawing convention for height or roofing, not evidence-bearing." },
    { id: "contested",    name: "Contested among excavators",  desc: "Specialists read the same evidence differently." },
    { id: "speculative",  name: "Speculative / unattested",    desc: "A plausible reading with no direct support." }
  ];
  const CAT_TIER = {
    bedrock: "excavated", excav: "excavated", church: "excavated", rubble: "excavated", modern: "excavated",
    plaster: "excavated", wall: "excavated",
    channel: "secured", islamic: "secured",
    colonnade: "textual",
    water: "inferred", fill: "inferred", pagan: "inferred",
    cave: "contested"
  };

  // SCH-04: the two rival proposals for where John 5's healing scene belongs —
  // the traditional twin-pool reading vs. the eastern-grotto reading argued by
  // Duprez and others. Coordinates match the herodian-phase geometry above.
  const GOSPEL_SETTINGS = {
    a: {
      label: "Southern pool", x: 52, y: 91, z: 0,
      case: "John 5:2 places the sick beneath five porticoes around the basins, and the Twin Pools carried exactly that colonnaded frame. On this reading the healing scene sits at the great reservoirs themselves."
    },
    b: {
      label: "Eastern grotto", x: 122, y: 68, z: 0,
      case: "The excavated caves and small basins east of the pools are architecturally consistent with a resort for the sick, and later tradition fixed the healing association there. On this reading the porticoes of John 5:2 frame the approach to that precinct, not the reservoirs."
    },
    note: "Both readings use the same excavated ground; they disagree about which part of it John 5 describes. This atlas does not adjudicate between them."
  };
  ...
  const pit   = (k, depth, water, label, cat, tier) => ({ kind: "pit", poly: P[k], depth, water, label, cat: cat || "plaster", tier });
  const box   = (k, z0, h, cat, label, tier, o) => Object.assign({ kind: "box", poly: P[k], z0, h, cat, label, tier }, o || {});
  const flat  = (k, z, cat, label, tier, o) => Object.assign({ kind: "flat", poly: P[k], z, cat, label, tier }, o || {});
  const colon = (a, b, n, h, label, tier) => ({ kind: "colonnade", a, b, n, h, cat: "colonnade", label, tier: tier || "textual" });

  const PERIODS = [
    {
      key: "iron", essay: "iron", name: "Iron Age Reservoir", span: "c. 800 – 600 BC", tick: "800 BC",
      strap: "A dam is thrown across the Beth Zeta valley, forming an early reservoir; some scholars link it to Isaiah's Upper Pool, though its water network is not securely mapped.",
      features: [
        ...
        flat("conduit", -0.5, "channel", "Rock-cut conduit, destination uncertain", "speculative")
      ],
      labels: [
        ...
        { x: 58, y: 146, z: 0, t: "conduit, destination uncertain", small: true }
      ]
    },
    {
      ...
      features: [
        ...
        flat("conduit", -0.5, "channel", "Conduit, destination uncertain", "speculative"),
        pit("caveA", 3, 1.2, "Rock cavity adapted as a stepped basin", "cave"),
        pit("caveB", 3, 1.2, "Rock cavity adapted as a stepped basin", "cave"),
        pit("bathB", 2.5, 1, "Small immersion basin — a miqveh?", "cave", "speculative")
      ],
      labels: [
        { x: 54, y: 40, z: 2, t: "UPPER POOL" },
    ...
    {
      key: "transition", essay: "transition", name: "Destruction & Reuse", span: "AD 70 – 130/135", tick: "AD 70",
      strap: "Jerusalem falls in AD 70; the site's later use before Hadrian's new city is incompletely known.",
      features: [
        pit("northPool", 13, 8, "Upper Pool — basins survive, porticoes gone", "fill"),
        pit("southPool", 13, 7, "Lower Pool — basins survive, porticoes gone", "fill"),
        box("dam", -13, 13.6, "wall", "Dam — the rock-cut basins outlast the built colonnades"),
        pit("caveA", 3.5, 1.4, "Rock-cut chamber and basin — use in this interval is unclear", "cave", "speculative"),
        pit("caveB", 3.5, 1.4, "Rock-cut chamber and basin — use in this interval is unclear", "cave", "speculative"),
        pit("caveC", 3.5, 1.4, "Rock-cut chamber and basin — use in this interval is unclear", "cave", "speculative")
      ],
      labels: [
        { x: 60, y: 36, z: 2, t: "UPPER POOL" },
        { x: 46, y: 98, z: 2, t: "LOWER POOL" },
        { x: 126, y: 102, z: 1, t: "evidence for this interval is incomplete", small: true }
      ]
    },
    {
      ...
      features: [
        ...
        colon([108, 86], [132, 86], 6, 8, "Temple portico — reconstructed", "inferred"),
        ...
        flat("votive", 4.1, "pagan", "Votive deposit — the marble ex-voto foot of Pompeia Lucilia", "excavated"),
    ...
    },
    {
      ...
      strap: "Early seventh-century damage, abandonment, and reuse — later tradition ties the basilica's ruin to the Persian conquest of 614, but the site-specific sequence is uncertain — then centuries of quiet: silt and one small chapel on the vaults.",
      features: [
    ...
    {
      key: "madrasa", essay: "madrasa", name: "Salahiyya Madrasa", span: "AD 1192 – 1856", tick: "1192",
      strap: "Saladin captures Jerusalem in 1187; in 1192 he endows the church as a law school, and the quarter slowly rises on its own debris.",
      features: [
  ...
  window.BETHESDA = Object.assign(window.BETHESDA || {}, {
    ...
    CAT_TIER: CAT_TIER,
    GOSPEL_SETTINGS: GOSPEL_SETTINGS
  });
})();
